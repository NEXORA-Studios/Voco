<script setup lang="ts">
    ...
    import { WordBankItem } from "../types/wordbank";
    ...
    const createDisabled = computed(
        () => inputBundleName.value.length > 0 && inputPackageName.value.length > 0 && !!inputFile.value
    );

    watch(
        () => inputFile.value,
        (newVal) => {
            const splitedName = newVal?.name.split(".");
            splitedName?.pop();
            const nameWithoutExtension = splitedName?.join(".") || "";
            [inputBundleName.value, inputPackageName.value] = nameWithoutExtension.split("#");
        }
    );

    async function createPackage() {
        if (!createDisabled.value || inputFile.value?.size === 0) return;
        const [packageInfo, data] = makeData(
            inputBundleName.value + "#" + inputPackageName.value,
            inputDescription.value,
            (await inputFile.value?.text()) || ""
        );
            ...
            <div class="modal-box w-xl! max-w-xl!">
                ...
                <section class="grid grid-cols-[1fr_3fr] grid-rows-3 gap-x-4 gap-y-2 items-center">
                    ...
                    <input
                        ...
                        type="file"
                        accept=".csv"
                        :placeholder="t('home.uploadFile')"
                        class="file-input w-full outline-none mt-2"
                        v-on:change="(e) => inputFile = (e.target as HTMLInputElement).files?.[0]" />
